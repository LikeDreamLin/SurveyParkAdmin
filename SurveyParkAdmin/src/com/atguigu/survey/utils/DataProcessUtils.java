...
public class DataProcessUtils {	
	...
	public static String calculatCodeByRoles(Set<Role> roles,Integer maxPox) {
		...
		if(!ValidateUtils.collectionValidate(roles)) return null;
		
		//1.遍历Role集合
		for (Role role : roles) {
			//2.获取每一个Role对象关联的Authority对象集合
			Set<Authority> authorities = role.getAuthorities();
			if(!ValidateUtils.collectionValidate(authorities)) continue ;
			...
			for (Authority authority : authorities) {
				...
				if(!ValidateUtils.collectionValidate(resources)) continue ;
	...
	public static JFreeChart generateChartByQsm(QuestionStatisticsModel qsm) {
		...
		String chartTitle = qsm.getQuestionName()+"【共"+qsm.getTotalCount()+"人次参与】";
	...
	public static String convertArrToStr(Object[] sourceArr) {
		...
		if(sourceArr.length == 1) return sourceArr[0] + "";
	...
	public static String removeLastComma(String oldContent) {
		...
		if(!ValidateUtils.stringValidate(oldContent)) return oldContent;
		
		if(!oldContent.contains(",")) return oldContent;
	...
	/**
	 * SELECT * FROM `logs` UNION SELECT * FROM logs_2016_3 UNION SELECT * FROM logs_2016_3
	 * @param tableNameList
	 * @return
	 */

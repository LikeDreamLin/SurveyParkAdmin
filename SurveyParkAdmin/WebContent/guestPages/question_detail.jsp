	...
	<s:else>
		<s:iterator value="questions">
			...
			<tr>
				...
				<td>
					<s:a namespace="/Guest" action="QuestionAction_toTypeChoosen?questionId=%{questionId}&surveyId=%{surveyId}&bagId=%{bagId}">编辑问题</s:a>
					<s:a namespace="/Guest" action="QuestionAction_remove?questionId=%{questionId}&surveyId=%{surveyId}">删除问题</s:a>
				</td>
			</tr>
			<%-- 二.Question的详细信息 --%>
			<%-- 1.单选题[选项、是否换行、其他项] --%>
			<s:if test="questionType == 0">
				<tr>
					<td colspan="2">
						<%-- Question类会自动将options字符串根据逗号拆分为数组，遍历数组即可 --%>
						<s:iterator value="optionsArray">
							<%-- 栈顶是单选题的每一个选项 --%>
							<input type="radio"/><s:property/><s:if test="br"><br/></s:if>
						</s:iterator>
						<s:if test="hasOther">
							<s:if test="otherType == 0"><input type="radio"/>其它</s:if>
							<s:if test="otherType == 1"><input type="text" class="shortInput"/>其它</s:if>
						</s:if>
					</td>
				</tr>
			</s:if>
			...
			<s:if test="questionType == 1">
				<tr>
					<td colspan="2">
						...
						<s:iterator value="optionsArray">
							<%-- 栈顶是多选题的每一个选项 --%>
							<input type="checkbox"/><s:property/><s:if test="br"><br/></s:if>
						</s:iterator>
						<s:if test="hasOther">

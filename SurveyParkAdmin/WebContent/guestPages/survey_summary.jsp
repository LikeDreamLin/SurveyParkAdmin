										...
										<tr>
											...
											<td>
												<s:if test="questionType == 0 || questionType == 1 || questionType == 2">
													<s:a namespace="/Guest" action="StatisticsAction_showNormalChart?questionId=%{questionId}">显示统计结果</s:a>
													<s:if test="hasOther && otherType == 1">
														<s:a namespace="/Guest" action="StatisticsAction_showOtherTextList?questionId=%{questionId}">以文本列表形式查看其他项的统计结果</s:a>
													</s:if>
												</s:if>
												<s:if test="questionType == 3">
													<s:a namespace="/Guest" action="StatisticsAction_showTextList?questionId=%{questionId}">以文本列表形式查看简答题的统计结果</s:a>
												</s:if>
												<s:if test="questionType == 4 || questionType == 5">
													<s:a namespace="/Guest" action="StatisticsAction_showNormalMatrixPage?questionId=%{questionId}">查看矩阵式问题的统计结果</s:a>
												</s:if>
												<s:if test="questionType == 6">
													<s:a namespace="/Guest" action="StatisticsAction_showOptionMatrixPage?questionId=%{questionId}">查看矩阵式问题的统计结果</s:a>
												</s:if>
											</td>
										</tr>
